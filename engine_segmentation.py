...
from dataset_segmentation import build_dataset
import shutil
...
import utils
from utils import get_config, dice_score, mean_dice_coef, torch_dice_coef_loss, iou,mean_iou, AverageMeter, ProgressMeter, step_decay
...
def Engine(args):
    ...
    if args.train_toggle:
        for trial in range(args.start_trial, args.trials+1):
            ...
            model = build_segmentation_model(num_classes, args).cuda()
            if torch.cuda.device_count() > 1:
                model = torch.nn.DataParallel(model)
            parameters = list(filter(lambda p: p.requires_grad, model.parameters()))
            loss = torch_dice_coef_loss
    ...
    with open(log_file, 'r') as reader, open(output_file, 'a') as writer:
        ...
        while experiment:
            ...
            score, metric = test(saved_model, test_loader, num_classes, args)
            
            print(">>{}: {} = {:.4f}".format(experiment, loss_type, score))
            writer.write("{}: {} = {:.4f}\n".format(experiment, loss_type, score))
            print(">>{}: {} = {:.4f}".format(experiment, mean_type, metric))
            writer.write("{}: {} = {:.4f}\n".format(experiment, mean_type, metric))
            mean_loss.append(metric) 
            experiment = reader.readline()
        
        mean_loss = np.array(mean_loss)
        
        print(">> All trials: m{}  = {}".format(loss_type, np.array2string(mean_loss, precision=4, separator=',')))
        writer.write("All trials: m{}  = {}\n".format(loss_type, np.array2string(mean_loss, precision=4, separator='\t')))
        print(">> Mean {} over All trials: = {:.4f}".format(loss_type, np.mean(mean_loss)))
        writer.write("Mean {} over All trials = {:.4f}\n".format(loss_type, np.mean(mean_loss)))
...
def test(checkpoint, data_loader_test, classes, args):
    model = build_segmentation_model(classes, args).cuda()
        ...
        for idx, (img, mask) in enumerate(data_loader_test):
            with torch.autocast("cuda"):
                ...
                if (idx + 1) % args.print_freq == 0:
                    print("Testing Step[{}/{}] ".format(idx + 1, len(data_loader_test)))
        print("Done testing iteration!")
    test_p = test_p.numpy()
    test_y = test_y.numpy()
    ...
    print(test_y.shape)
    if args.dataset == "chestxdet":
        score = iou(test_y, test_p)
        metric = mean_iou(test_y, test_p)
    else:
        score = dice_score(test_y, test_p)
        metric = mean_dice_coef(test_y > 0.5, test_p > 0.5)
